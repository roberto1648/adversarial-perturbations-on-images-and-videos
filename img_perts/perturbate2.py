...
def main(
    ...
    :param class_index: (int, int list, or "top5") the (vgg)
    model output(s) to be minimized. "max" minimizes the 5
    maximum classes in the original image.
    :param minimize_class: (Bool) Whether to minimize (True) or
    maximize (False) the indicated class.
...
def get_optimum_perturbation(# todo: fix for categories
    ...
    for i in tqdm(range(epochs)):
        ...
        if minimize_class is False: sc_term = 1 - sc_term # maximize class score
